...
func InitBuilder(dir, file string, modified time.Time, asm, suffix, title, linkTitle, desc string) (string, io.WriteCloser, error) {
  ...
  if err := util.GenerateReferenceIndices(buildFileName, modified); err != nil {
    return "", nil, err
  }

  if err := util.GenerateCustomIndexFile(path.Join(dir, asm, file, "_index.html"), modified, func(indexFileName string, fileTime time.Time) error {
