...
type Writer struct {
	...
	blocks []string       // Slice of blocks to close
}
...
func (w *Writer) Block(start, end string) *Writer {
	w.WriteString(start)
	w.blocks = append(w.blocks, end)
	return w
}

func (w *Writer) EndAllBlocks() *Writer {
	for len(w.blocks) > 0 {
		w.EndBlock()
	}
	return w
...
func (w *Writer) EndBlock() *Writer {
	l := len(w.blocks)
	if l > 0 {
		n := w.blocks[l-1]
		w.blocks = w.blocks[:l-1]
		w.WriteString(n)
	}
	return w
}
